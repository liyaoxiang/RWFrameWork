    ...
    public partial struct PassivityConfig
    {
        public static void DeserializeByAddressable(string directory)
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                PassivityConfig data = (PassivityConfig)dataObject.ToObject(typeof(PassivityConfig));
                datas.Add(data);
        ...
        {
            string bundleName = $"{subFolder}/PassivityConfig.bytes".ToLower();
        ...
        private static List<PassivityConfig> datas;
        private static Dictionary<int, int> indexMap;
        public static PassivityConfig ByID(int id)
        {
            if (id <= 0)
            {
                return Null;
            ...
            {
                throw new System.Exception($"PassivityConfig找不到ID:{id}");
            }
            return ByIndex(index);
        }
        public static PassivityConfig ByIndex(int index)
        {
            return datas[index];
        }
        public bool IsNull { get; private set; }
        public static PassivityConfig Null { get; } = new PassivityConfig() { IsNull = true }; 
